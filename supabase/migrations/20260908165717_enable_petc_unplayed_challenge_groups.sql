...
begin
  ...
  if p_club_slug in ('seoultech', 'petc') then
    -- A group starts at each active player with a current-season confirmed match.
    -- Count group boundaries in (defender, challenger], so unplayed players
    -- share the position of the preceding played player. Same-group distance is 0.
    ...
    where season_player.season_id = v_season_id
      ...
      and exists (
        select 1
        from public.matches as participation
        where participation.season_id = v_season_id
          and participation.status = 'confirmed'
          and (
            participation.challenger_player_id = season_player.player_id
            or participation.defender_player_id = season_player.player_id
          )
      );
  else
    select count(*)::integer
  ...
  if v_active_distance > v_challenge_range then

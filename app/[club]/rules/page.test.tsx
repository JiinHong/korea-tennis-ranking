...
describe("ClubRulesPage", () => {
  it("실제 시스템과 일치하는 핵심 운영 규칙을 안내한다", async () => {
    ...
    expect(
      within(challengeSection).getByText(/실제 순위는 그대로 유지/),
    ).toBeDefined();
    expect(
      within(challengeSection).getByText(
  ...
  it("서울과기대 시즌 3 시작 기준과 시즌 2 기록을 함께 제공한다", async () => {
    ...
    expect(
      within(seasonSection).getByText(/신규 선수는 가나다순/),
    ).toBeDefined();
    expect(
      screen
        .getByRole("link", { name: "랭킹으로 돌아가기" })
        .getAttribute("href"),
    ).toBe("/seoultech");
  });

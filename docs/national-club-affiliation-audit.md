...
- 조사일: 2026-07-23
- 조사 대상: `data/national-ranking/v1/dataset.json`의 64개 동아리
- 현재 반영 데이터 버전: `sources-2026-07-26-v17`
...
| 64 | 용인대학교 ACE | **용인대학교 ACE** | 대학 테니스 동아리 | 사용자 확인 | 운영자가 제공한 2023 양구 남자부 대학-동아리 대조표에서 `용인대 Ace`의 소속을 확인했다. |
...
KUTA·LOVICE에는 과거 KUTA, LOVICE(러비스), 셔틀로 이미 귀속된 51개
결과가 들어가며, IMPACT에는 IMPACT·임팩트로 이미 귀속된 12개 결과가
들어간다. 출전명과 출처는 변경하지 않아 나중에 다시 분리할 수 있다.
단, 원자료에 `경희대 A/B/C`처럼만 적혀 두 그룹 중 어느 쪽인지 판별할
수 없는 17개 결과는 임의로 합치지 않고 미확정 상태로 둔다.
...
두 조직은 실제로 서로 다른 동아리다. 다만 운영자 지시에 따라 과거
`러시앤`, `테사랑`, `티샷`, `명지대 A/B` 결과를 현재 랭킹에서는
`명지대학교 MJTA·MTC` 하나로 합산한다. 향후 원자료별 소속이 확인되면
두 동아리로 다시 분리할 수 있도록 과거 출전명은 별칭과 결과 원문에
그대로 보존한다.
...
3. 경희대 셔틀의 캠퍼스·학과
4. 성균관대 공자·맹자·순자·SIT가 모두 STC 팀명인지, 캠퍼스별 조직인지
5. 금오공대 KOTC와 현재 공식 명칭 `금오테니스`가 같은 조직인지
